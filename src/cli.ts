...
program
  .name('jira-to-pr')
  .description(
    'Uses an AI model to generate minimal code changes from a Jira ticket.'
  )
  .version('1.0.2');
  ...
  .action(async (options) => {
    try {
      ...
      for (const ticket of tickets.slice(0, limit)) {
        ...
        console.log(
          chalk.dim(
            `    ${ticket.issueType} | ${ticket.priority} | ${ticket.status} | ${hasAC}`
          )
        );
        console.log();
      }
...
program
  ...
  .action(() => {
    ...
    console.log(
      `  API Token: ${
        config.jira.apiToken ? chalk.green('✓ set') : chalk.red('✗ not set')
      }`
    );
    console.log(
      `  Project Key: ${config.jira.projectKey || chalk.dim('(not set)')}`
    );
    console.log(
      `  Label Filter: ${config.jira.labelFilter || chalk.dim('(none)')}`
    );

    console.log(chalk.cyan('\nGitHub:'));
    console.log(
      `  Token: ${
        config.github.token ? chalk.green('✓ set') : chalk.red('✗ not set')
      }`
    );
    ...
    if (config.ai.provider !== 'ollama') {
      console.log(
        `  API Key: ${
          config.ai.apiKey ? chalk.green('✓ set') : chalk.red('✗ not set')
        }`
      );
    }
    ...
    console.log(
      `  Require Single Ticket: ${config.safety.requireSingleTicket}`
    );
    console.log();
  });
